...
from .preprocessing import parse_list, clean_and_normalize, merge_attack_classes, meta_features_df
...
def main():
    ...
    df["attack_type_parsed"] = df["attack_type"].apply(parse_list)
    df["attack_type_parsed"] = df["attack_type_parsed"].apply(clean_and_normalize)

    # notebook merged experiment scelto come pipeline finale
    df["attack_type_parsed"] = df["attack_type_parsed"].apply(merge_attack_classes)

    df = df[df["attack_type_parsed"].map(len) > 0].copy()

    X = df[["text_clean"]].copy()

    mlb = MultiLabelBinarizer()
    Y = mlb.fit_transform(df["attack_type_parsed"])
    ...
    print("Attack type model salvato correttamente.")
