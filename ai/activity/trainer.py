...
from .preprocessing import parse_list, clean_and_normalize, merge_activity_classes, meta_features_df


BASE_DIR = Path(__file__).resolve().parents[2]
DATA_PATH = BASE_DIR / "dataset" / "telegram_dataset_activity_classification.xlsx"
...
def main():
    ...
    df["activity_list"] = df["activity_labels"].apply(parse_list)
    df["activity_list"] = df["activity_list"].apply(clean_and_normalize)

    # notebook merged experiment scelto come pipeline finale
    df["activity_list"] = df["activity_list"].apply(merge_activity_classes)

    df = df[df["activity_list"].map(len) > 0].copy()

    X = df[["text_clean"]].copy()

    mlb = MultiLabelBinarizer()
    Y = mlb.fit_transform(df["activity_list"])
    ...
    print("Activity model salvato correttamente.")
